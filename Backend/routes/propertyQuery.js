...
class GetPropertyQuery extends SQLQuery {
  constructor(fields) {
    ...
    this.furnished = new mQueryField("furnishing = ?", fields.furnished, [
      "None",
      "Semi",
      "Full",
    ]);
    this.curfew = new queryField("curfew = ?", fields.curfew);
    this.type = new mQueryField("lot_type = ?", fields.type, [
      "Condominium",
      "Dormitory",
      "Apartment",
      "Boarding House",
    ...
    this.stay = new mQueryField(
      "(min_month_stay >= ? AND min_month_stay <= ?)",
      fields.stay,
      [
        [0, 6],
        [7, 12],
    ...
    this.inclusions = new mQueryField(
      "FIND_IN_SET(?, inclusion)",
      fields.inclusions,
      ["Electricity", "Water", "WiFi", "Kitchen", "Parking"]
    );
...
class Property extends SQLQuery {
  // used by update and create property
  constructor(clause, fields) {
    ...
    this.occupancy = new queryField("occupancy", fields.occupancy);
    this.curfew = new queryField("curfew", fields.curfew);
...
class UpdatePropertyQuery extends Property {
  ...
  formatFilters(key) {
    if (key == "pid" || key == "lid")
      // these should only appear after WHERE keyword
      return;
    super.formatFilters(key);
  }
  build() {
...
class DeletePropertyQuery extends SQLQuery {
  ...
  build() {
    ...
    this.queryValues.push(this.pid.getValue());
    if (this.lid.getValue() != ADMIN) {
      // authenticate further if not admin
      this.sql += " AND landlord_id = ?";
      this.queryValues.push(this.lid.getValue());
    }
  }
